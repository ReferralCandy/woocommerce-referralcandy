...
Testing is manual: activate plugin, fill API keys under WooCommerce > ReferralCandy (wc-admin React page, `admin.php?page=wc-admin&path=/referralcandy`), place order, move it to configured status, check order note "Order sent to ReferralCandy".
...
One codebase, two builds. `woocommerce-referralcandy.php` starts with three flavor defines — `WC_REFERRALCANDY_SUFFIX` (`''` / `'_staging'`), `WC_REFERRALCANDY_LABEL`, `WC_REFERRALCANDY_API_BASE` — and everything flavor-specific derives from them at runtime: integration id + option key (`woocommerce_referralcandy[_staging]_settings`), REST namespace (`referralcandy[-staging]/v1`), wc-admin path, checkout field ids/names, labels, order note. Order meta (`rc_aic`, `rc_loc`, `rc_accepts_marketing`) and the `rc_referrer_id` cookie are deliberately shared so both flavors see the same attribution.

`scripts/package.mjs staging` rewrites those three defines (the staging API base comes from `WC_REFERRALCANDY_STAGING_API_BASE` in a gitignored `.env` or the shell — never commit it, the repo is public; see `.env.example`), the `Plugin Name`, and suffixes every PHP class/function/constant (`WC_Referralcandy_Staging`, `RC_Order_Staging`, `wc_referralcandy_staging_*`, `WC_REFERRALCANDY_STAGING_*`) so the staging plugin (folder `referralcandy-for-woocommerce`) can be active next to production on one store. Add new flavor-dependent values by deriving from the defines, not by adding replacements; add new global PHP symbols using the existing `WC_Referralcandy` / `RC_` / `wc_referralcandy_` / `WC_REFERRALCANDY_` prefixes so the rename map keeps catching them.

The React bundle is identical for both flavors: `RC_Admin::enqueue` pushes `{ id, title, path, restPath }` onto `window.wcReferralCandyPages` via an inline script, both plugins share the `wc-referralcandy-admin` handle on purpose, and `src/index.js` registers one page per config.

## Architecture

Four PHP files plus one JS entry carry the whole plugin:
...
- `includes/class-rc-admin.php` — `RC_Admin`. Registers the wc-admin page (`wc_admin_register_page`, id/path `referralcandy`), enqueues `build/index.js` on wc-admin pages using `build/index.asset.php` deps, and exposes REST `referralcandy/v1/settings` (GET/POST, `manage_woocommerce`); pushes the page config for the JS bundle. POST accepts partial objects: defaults ← stored ← submitted, then `validate_settings()`.
- `includes/class-rc-order.php` — `RC_Order`. Builds the purchase payload from a `WC_Order` and POSTs to `https://my.referralcandy.com/api/v1/purchase.json`. Signature is `md5(secret_key . concatenated sorted "key=value" pairs)`; `prepParams()` exists because `http_build_query` mangles the `timestamp` key — keep using it. Adds order note on success.
- `src/index.js` — registers the route via `addFilter('woocommerce_admin_pages_list')` (`navArgs.id` must match the PHP page id) and renders the settings form from the REST `fields` schema with `@wordpress/components`. Only `@wordpress/*` packages are imported, so default `wp-scripts` dependency extraction suffices — no `webpack.config.js`. If `@woocommerce/*` packages are ever imported, add `@woocommerce/dependency-extraction-webpack-plugin`.
