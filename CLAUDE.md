...
- `src/` — React app mounted by `index.js` into the root div. `app.js` = hash router (`#/`, `#/settings/<group>`, `#/help`) + settings load/save state; `shell.js` = dark 300px sidebar (Back to WP Admin, nav, settings sub-nav, footer) + white content card with header/Save; `groups.js` = which PHP field keys form which settings sub-page (unknown keys fall into "Other"); `pages/` = overview (hero + status checks), settings (rows: title/description left, control right, Tips column), help. `style.scss` → `build/style-index.css`. Imports `@wordpress/*` externals plus `@wordpress/icons` (bundled, tree-shaken); default `wp-scripts` config, no `webpack.config.js`.
